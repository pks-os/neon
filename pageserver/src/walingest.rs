...
enum_pgversion! {CheckpointVersion, pgv::CheckPoint}

impl CheckpointVersion {
    fn encode(&self) -> Result<Bytes, SerializeError> {
        enum_pgversion_dispatch!(self, CheckpointVersion, cp, { cp.encode() })
    }

    fn update_next_xid(&mut self, xid: u32) -> bool {
        enum_pgversion_dispatch!(self, CheckpointVersion, cp, { cp.update_next_xid(xid) })
    }

    pub fn update_next_multixid(&mut self, multi_xid: u32, multi_offset: u32) -> bool {
        enum_pgversion_dispatch!(self, CheckpointVersion, cp, {
...
impl WalIngest {
    pub async fn new(
        ...
        let checkpoint = dispatch_pgversion!(pgversion, {
            ...
            <pgv::CheckPoint as Into<CheckpointVersion>>::into(checkpoint)
        });
    ...
    pub async fn ingest_record(
        ...
        match decoded.xl_rmid {
            ...
            pg_constants::RM_XLOG_ID => {
                ...
                trait Merge {
                    fn from(&mut self, other: &Self) -> bool;
                }

                impl Merge for postgres_ffi::v14::CheckPoint {
                    fn from(&mut self, _other: &Self) -> bool {
                        false
                    }
                }
                impl Merge for postgres_ffi::v15::CheckPoint {
                    fn from(&mut self, _other: &Self) -> bool {
                        false
                    }
                }
                impl Merge for postgres_ffi::v16::CheckPoint {
                    fn from(&mut self, _other: &Self) -> bool {
                        false
                    }
                }
    ...
    fn warn_on_ingest_lag(
        ...
        let ts = enum_pgversion_dispatch!(&self.checkpoint, CheckpointVersion, _cp, {
    ...
    async fn ingest_clog_truncate_record(
        ...
        enum_pgversion_dispatch!(&mut self.checkpoint, CheckpointVersion, cp, {
            if cp.oldestXid != xlrec.oldest_xid || cp.oldestXidDB != xlrec.oldest_xid_db {
        ...
        let latest_page_number =
            enum_pgversion_dispatch!(self.checkpoint, CheckpointVersion, cp, { cp.nextXid.value })
                as u32
        ...
        let precedes = dispatch_pgversion!(modification.tline.pg_version, {
    ...
    async fn ingest_multixact_truncate_record(
        ...
        let (modified, maxsegment, startsegment, endsegment) =
            enum_pgversion_dispatch!(&mut self.checkpoint, CheckpointVersion, cp, {
                let modified = cp.oldestMultiDB != xlrec.oldest_multi_db
                ...
                (modified, maxsegment, startsegment, endsegment)
            });
